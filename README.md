...
- **Claude Code (claude.ai/code)** 를 활용하여 개발을 진행했습니다.
- 코드 리뷰, 테스트 코드 작성, 버그 발견(Processor readOnly 트랜잭션 버그, 중복 저장 시 트랜잭션 오염 문제), 문서 작성에 활용했습니다.
- 설계 결정(헥사고날 아키텍처, 이벤트 리스너 구조, 재시도 정책 등)은 직접 수행하였으며, AI는 구현 보조 및 검증 역할을 했습니다.
